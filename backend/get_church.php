...
if (isset($_POST['user_id'])
	...
	&& isset($_POST['type'])) {
	// type = 0 -> get all church
	// type = 1 -> get church filter city
	...
	$query = "SELECT church.id as church_id, church.name as church_name, church.status as status,
			...
			WHERE church.id_province = province.id AND church.id_ward = ward.id 
			AND church.id_districts = districts.id AND church.id = open_time.id_church";
	if($type == 1){		
		$query .= " AND province.id = ".$id_province;			
	}	
